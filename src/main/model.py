...
from src.common.nets.loss import (CoordLoss, EdgeLengthLoss, NormalVectorLoss,
                                  ParamLoss)
from src.common.nets.module import MeshNet, ParamRegressor, Pose2Feat, PoseNet
from src.common.nets.resnet import ResNetBackbone
from src.common.utils.mano import MANO
...
class Model(nn.Module):
    def __init__(self, pose_backbone, pose_net, pose2feat, mesh_backbone, mesh_net, param_regressor):
        ...
        if cfg.stage == 'lixel':
            self.trainable_modules = [
                self.pose_backbone, self.pose_net, self.pose2feat, self.mesh_backbone, self.mesh_net]
        else:
    ...
    def make_gaussian_heatmap(self, joint_coord_img):
        ...
        heatmap = torch.exp(-(((xx-x)/cfg.sigma)**2)/2 -
                            (((yy-y)/cfg.sigma)**2)/2 - (((zz-z)/cfg.sigma)**2)/2)
        return heatmap
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        with cm:
            ...
            #     inputs = torch.cat((inputs['rgb'], inputs['depth']), dim=1)
            # else:
            inputs = inputs['img']

            shared_img_feat, pose_img_feat = self.pose_backbone(inputs)
            joint_coord_img = self.pose_net(pose_img_feat)

            # make 3D heatmap from posenet output and convert to image feature
            with torch.no_grad():
                joint_heatmap = self.make_gaussian_heatmap(
                    joint_coord_img.detach())
            shared_img_feat = self.pose2feat(shared_img_feat, joint_heatmap)

            # meshnet forward
            _, mesh_img_feat = self.mesh_backbone(
                shared_img_feat, skip_early=True)
            mesh_coord_img = self.mesh_net(mesh_img_feat)

            # joint coordinate outputs from mesh coordinates
            joint_img_from_mesh = torch.bmm(torch.from_numpy(self.joint_regressor).cuda()[
                                            None, :, :].repeat(mesh_coord_img.shape[0], 1, 1), mesh_coord_img)
            mesh_coord_cam = None

        if cfg.stage == 'param':
            # parameter regression
            pose_param, shape_param = self.param_regressor(
                joint_img_from_mesh.detach())

            # get mesh and joint coordinates
            mesh_coord_cam, _ = self.human_model_layer(pose_param, shape_param)
            joint_coord_cam = torch.bmm(torch.from_numpy(self.joint_regressor).cuda()[
                                        None, :, :].repeat(mesh_coord_cam.shape[0], 1, 1), mesh_coord_cam)
        ...
        if mode == 'train':
            ...
            if cfg.stage == 'lixel':
                loss['joint_fit'] = self.coord_loss(
                    joint_coord_img, targets['fit_joint_img'], meta_info['fit_joint_trunc'] * meta_info['is_valid_fit'][:, None, None])
                loss['joint_orig'] = self.coord_loss(
                    joint_coord_img, targets['orig_joint_img'], meta_info['orig_joint_trunc'], meta_info['is_3D'])
                loss['mesh_fit'] = self.coord_loss(
                    mesh_coord_img, targets['fit_mesh_img'], meta_info['fit_mesh_trunc'] * meta_info['is_valid_fit'][:, None, None])
                loss['mesh_joint_orig'] = self.coord_loss(
                    joint_img_from_mesh, targets['orig_joint_img'], meta_info['orig_joint_trunc'], meta_info['is_3D'])
                loss['mesh_joint_fit'] = self.coord_loss(
                    joint_img_from_mesh, targets['fit_joint_img'], meta_info['fit_joint_trunc'] * meta_info['is_valid_fit'][:, None, None])
                loss['mesh_normal'] = self.normal_loss(
                    mesh_coord_img, targets['fit_mesh_img'], meta_info['fit_mesh_trunc'] * meta_info['is_valid_fit'][:, None, None]) * cfg.normal_loss_weight
                loss['mesh_edge'] = self.edge_loss(
                    mesh_coord_img, targets['fit_mesh_img'], meta_info['fit_mesh_trunc'] * meta_info['is_valid_fit'][:, None, None])
            else:
                loss['pose_param'] = self.param_loss(
                    pose_param, targets['pose_param'], meta_info['is_valid_fit'][:, None])
                loss['shape_param'] = self.param_loss(
                    shape_param, targets['shape_param'], meta_info['is_valid_fit'][:, None])
                loss['joint_orig_cam'] = self.coord_loss(
                    joint_coord_cam, targets['orig_joint_cam'], meta_info['orig_joint_valid'] * meta_info['is_3D'][:, None, None])
                loss['joint_fit_cam'] = self.coord_loss(
                    joint_coord_cam, targets['fit_joint_cam'], meta_info['is_valid_fit'][:, None, None])
            return loss
        else:
            # test output
            out = {}
...
def get_model(vertex_num, joint_num, mode, rgbd=False):
    ...
    # number of input channels=4, if using RGBD input
    if rgbd:
        print(f"Using RGBD input")
        pose_backbone.conv1 = nn.Conv2d(4, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)

    model = Model(pose_backbone, pose_net, pose2feat, mesh_backbone, mesh_net, param_regressor)
    return model

    model = Model(pose_backbone, pose_net, pose2feat,
                  mesh_backbone, mesh_net, param_regressor)
    return model
