...
from src.common.utils.vis import save_obj, vis_keypoints, vis_mesh
...
class PW3D(torch.utils.data.Dataset):
    def __init__(self, transform, data_split, rgbd=False):
        ...
        self.data_path = osp.join(
            './datasets/i2l/data/PW3D', 'data')
        self.human_bbox_root_dir = osp.join(
            './datasets/i2l/data/PW3D', 'rootnet_output', 'bbox_root_pw3d_output.json')
        self.rgbd = rgbd
        ...
        self.h36m_eval_joint = (1, 2, 3, 4, 5, 6, 8, 10,
                                11, 12, 13, 14, 15, 16)
        self.h36m_joint_regressor = np.load(osp.join(
            '/project/aksoy-lab/Mahesh/Courses/CMPT_766/rgbd-i2l/src/data/Human36M', 'J_regressor_h36m_correct.npy'))
    ...
    def load_data(self):
        ...
        if self.data_split == 'test' and not cfg.use_gt_info:
            ...
            for i in range(len(annot)):
                ...
                bbox_root_result[ann_id] = {'bbox': np.array(
        ...
        for aid in db.anns.keys():
            ...
            img_path = osp.join(self.data_path, 'imageFiles',
                                sequence_name, img_name)
    ...
    def get_smpl_coord(self, smpl_param):
        pose, shape, trans, gender = smpl_param['pose'], smpl_param[
            'shape'], smpl_param['trans'], smpl_param['gender']
        ...
        smpl_mesh_coord, smpl_joint_coord = self.smpl.layer[gender](
    ...
    def __getitem__(self, idx):
        ...
        if self.rgbd:
            depth_path = img_path.replace(
                '/imageFiles', '/dpt').replace('.jpg', '.png')
            depth = load_depth(depth_path)

        # img
        img = load_img(img_path)
        img, img2bb_trans, bb2img_trans, _, _, depth = augmentation(
            img, bbox, self.data_split, depth=depth)
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            mesh_out_img[:, 2] = (
                mesh_out_img[:, 2] / cfg.output_hm_shape[0] * 2. - 1) * (cfg.bbox_3d_size / 2)
            ...
            eval_result['mpjpe_lixel'].append(np.sqrt(np.sum(
                (pose_coord_out_h36m - pose_coord_gt_h36m)**2, 1)).mean() * 1000)  # meter -> milimeter
            eval_result['pa_mpjpe_lixel'].append(np.sqrt(np.sum(
                (pose_coord_out_h36m_aligned - pose_coord_gt_h36m)**2, 1)).mean() * 1000)  # meter -> milimeter
            ...
            if cfg.stage == 'param':
                ...
                pose_coord_out_h36m_aligned = rigid_align(
                    pose_coord_out_h36m, pose_coord_gt_h36m)
                eval_result['mpjpe_param'].append(np.sqrt(np.sum(
                    (pose_coord_out_h36m - pose_coord_gt_h36m)**2, 1)).mean() * 1000)  # meter -> milimeter
                eval_result['pa_mpjpe_param'].append(np.sqrt(np.sum(
                    (pose_coord_out_h36m_aligned - pose_coord_gt_h36m)**2, 1)).mean() * 1000)  # meter -> milimeter
