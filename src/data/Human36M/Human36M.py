...
from src.common.utils.preprocessing import (augmentation, generate_patch_image,
                                            get_bbox, load_depth, load_img,
                                            process_bbox)
from src.common.utils.smpl import SMPL
from src.common.utils.transforms import (cam2pixel, pixel2cam, rigid_align,
                                         transform_joint_to_other_db,
                                         world2cam)
from src.common.utils.vis import save_obj, vis_mesh
from src.main.config import cfg


class Human36M(torch.utils.data.Dataset):
    def __init__(self, transform, data_split, rgbd=False):
        self.transform = transform
        self.data_split = data_split
        self.img_dir = osp.join('./mkk15/datasets/i2l/data/Human36M', 'images')
        self.annot_path = osp.join('./mkk15/datasets/i2l/data/Human36M', 'annotations')
        self.human_bbox_root_dir = osp.join('./mkk15/datasets/i2l/data/Human36M', 'rootnet_output', 'bbox_root_human36m_output.json')
        ...
        self.h36m_joints_name = ('Pelvis', 'R_Hip', 'R_Knee', 'R_Ankle', 'L_Hip', 'L_Knee', 'L_Ankle', 'Torso',
                                 'Neck', 'Nose', 'Head_top', 'L_Shoulder', 'L_Elbow', 'L_Wrist', 'R_Shoulder', 'R_Elbow', 'R_Wrist')
        self.h36m_flip_pairs = ((1, 4), (2, 5), (3, 6),
                                (14, 11), (15, 12), (16, 13))
        self.h36m_skeleton = ((0, 7), (7, 8), (8, 9), (9, 10), (8, 11), (11, 12), (12, 13), (
        ...
        self.h36m_joint_regressor = np.load(osp.join('/project/aksoy-lab/Mahesh/Courses/CMPT_766/rgbd-i2l/src/data/Human36M', 'J_regressor_h36m_correct.npy'))
    ...
    def load_data(self):
        ...
        if self.data_split == 'test' and not cfg.use_gt_info:
            ...
            for i in range(len(annot)):
                bbox_root_result[str(annot[i]['image_id'])] = {'bbox': np.array(
                    annot[i]['bbox']), 'root': np.array(annot[i]['root_cam'])}
        else:
        ...
        for aid in db.anns.keys():
            ...
            frame_idx = img['frame_idx']
            try:
                smpl_param = smpl_params[str(subject)][str(
                    action_idx)][str(subaction_idx)][str(frame_idx)]
            except KeyError:
                smpl_param = None
            ...
            joint_world = np.array(joints[str(subject)][str(action_idx)][str(
                subaction_idx)][str(frame_idx)], dtype=np.float32)
            ...
            if self.data_split == 'test' and not cfg.use_gt_info:
                ...
                bbox = process_bbox(
                    np.array(ann['bbox']), img['width'], img['height'])
                if bbox is None:
                    continue
    ...
    def get_smpl_coord(self, smpl_param, cam_param, do_flip, img_shape):
        ...
        R, t = np.array(cam_param['R'], dtype=np.float32).reshape(3, 3), np.array(
            cam_param['t'], dtype=np.float32).reshape(3)  # camera rotation and translation
        ...
        if do_flip:
            ...
            # multiply -1 to y and z axis of axis-angle
            smpl_pose[:, 1:3] *= -1
        smpl_pose = smpl_pose.view(1, -1)

        # get mesh and joint coordinates
        smpl_mesh_coord, smpl_joint_coord = self.smpl.layer['neutral'](
            smpl_pose, smpl_shape)

        # incorporate face keypoints
        smpl_mesh_coord = smpl_mesh_coord.numpy().astype(np.float32).reshape(-1, 3)
        smpl_joint_coord = smpl_joint_coord.numpy().astype(np.float32).reshape(-1, 3)
        smpl_face_kps_coord = smpl_mesh_coord[self.face_kps_vertex, :].reshape(
            -1, 3)
        smpl_joint_coord = np.concatenate(
            (smpl_joint_coord, smpl_face_kps_coord))
        ...
        smpl_trans = np.dot(R, smpl_trans[:, None]).reshape(
            1, 3) + t.reshape(1, 3)/1000
        root_joint_coord = smpl_joint_coord[self.root_joint_idx].reshape(1, 3)
        smpl_trans = smpl_trans - root_joint_coord + \
            np.dot(R, root_joint_coord.transpose(1, 0)).transpose(1, 0)
        ...
        if do_flip:  # avg of old and new root joint should be image center.
            ...
            flip_trans_x = 2 * (((img_shape[1] - 1)/2. - princpt[0]) / focal[0] * (
                smpl_joint_coord[self.root_joint_idx, 2] * 1000)) / 1000 - 2 * smpl_joint_coord[self.root_joint_idx][0]
    ...
    def get_fitting_error(self, h36m_joint, smpl_mesh, do_flip):
        h36m_joint = h36m_joint - \
            h36m_joint[self.h36m_root_joint_idx, None, :]  # root-relative
        if do_flip:
            h36m_joint[:, 0] = -h36m_joint[:, 0]
            for pair in self.h36m_flip_pairs:
                h36m_joint[pair[0], :], h36m_joint[pair[1], :] = h36m_joint[pair[1], :].copy(
                ), h36m_joint[pair[0], :].copy()

        h36m_from_smpl = np.dot(self.h36m_joint_regressor, smpl_mesh)
        h36m_from_smpl = h36m_from_smpl - \
            np.mean(h36m_from_smpl, 0)[
                None, :] + np.mean(h36m_joint, 0)[None, :]  # translation alignment
    ...
    def __getitem__(self, idx):
        ...
        depth = None
        if self.rgbd:
            depth_path = img_path.replace('/images', '/dpt').replace('.jpg', '.png')
            depth = load_depth(depth_path)
         
        # img
        img = load_img(img_path)
        img, img2bb_trans, bb2img_trans, rot, do_flip, depth = augmentation(img, bbox, self.data_split, depth=depth)
        ...
        if self.data_split == 'train':
            ...
            h36m_joint_cam = h36m_joint_cam - \
                h36m_joint_cam[self.h36m_root_joint_idx,
                               None, :]  # root-relative
            ...
            h36m_joint_img_xy1 = np.concatenate(
                (h36m_joint_img[:, :2], np.ones_like(h36m_joint_img[:, :1])), 1)
            h36m_joint_img[:, :2] = np.dot(
                img2bb_trans, h36m_joint_img_xy1.transpose(1, 0)).transpose(1, 0)
            h36m_joint_img[:, 0] = h36m_joint_img[:, 0] / \
                cfg.input_img_shape[1] * cfg.output_hm_shape[2]
            h36m_joint_img[:, 1] = h36m_joint_img[:, 1] / \
                cfg.input_img_shape[0] * cfg.output_hm_shape[1]
            h36m_joint_img[:, 2] = h36m_joint_img[:, 2] - \
                h36m_joint_img[self.h36m_root_joint_idx][2]  # root-relative
            # change cfg.bbox_3d_size from meter to milimeter
            h36m_joint_img[:, 2] = (
                h36m_joint_img[:, 2] / (cfg.bbox_3d_size * 1000 / 2) + 1)/2. * cfg.output_hm_shape[0]
            ...
            h36m_joint_img = transform_joint_to_other_db(
                h36m_joint_img, self.h36m_joints_name, self.joints_name)
            h36m_joint_cam = transform_joint_to_other_db(
                h36m_joint_cam, self.h36m_joints_name, self.joints_name)
            h36m_joint_valid = transform_joint_to_other_db(
                h36m_joint_valid, self.h36m_joints_name, self.joints_name)
            h36m_joint_trunc = transform_joint_to_other_db(
                h36m_joint_trunc, self.h36m_joints_name, self.joints_name)
            ...
            if smpl_param is not None:
                ...
                smpl_coord_img_xy1 = np.concatenate(
                    (smpl_coord_img[:, :2], np.ones_like(smpl_coord_img[:, :1])), 1)
                smpl_coord_img[:, :2] = np.dot(
                    img2bb_trans, smpl_coord_img_xy1.transpose(1, 0)).transpose(1, 0)[:, :2]
                smpl_coord_img[:, 2] = smpl_coord_img[:, 2] - \
                    smpl_coord_cam[self.vertex_num + self.root_joint_idx][2]
                # coordinates voxelize
                smpl_coord_img[:, 0] = smpl_coord_img[:, 0] / \
                    cfg.input_img_shape[1] * cfg.output_hm_shape[2]
                smpl_coord_img[:, 1] = smpl_coord_img[:, 1] / \
                    cfg.input_img_shape[0] * cfg.output_hm_shape[1]
                # change cfg.bbox_3d_size from meter to milimeter
                smpl_coord_img[:, 2] = (
                    smpl_coord_img[:, 2] / (cfg.bbox_3d_size * 1000 / 2) + 1)/2. * cfg.output_hm_shape[0]
                ...
                is_valid_fit = True
                error = self.get_fitting_error(
                    data['joint_cam'], smpl_mesh_cam, do_flip)
                if error > self.fitting_thr:
                    is_valid_fit = False

            else:
                smpl_joint_img = np.zeros(
                    (self.joint_num, 3), dtype=np.float32)  # dummy
                smpl_joint_cam = np.zeros(
                    (self.joint_num, 3), dtype=np.float32)  # dummy
                smpl_mesh_img = np.zeros(
                    (self.vertex_num, 3), dtype=np.float32)  # dummy
                smpl_pose = np.zeros((72), dtype=np.float32)  # dummy
                smpl_shape = np.zeros((10), dtype=np.float32)  # dummy
                smpl_joint_trunc = np.zeros(
                    (self.joint_num, 1), dtype=np.float32)  # dummy
                smpl_mesh_trunc = np.zeros(
                    (self.vertex_num, 1), dtype=np.float32)  # dummy
                is_valid_fit = False
            ...
            # h36m coordinate
            h36m_joint_cam = np.dot(rot_aug_mat, h36m_joint_cam.transpose(
            ...
            smpl_joint_cam = smpl_joint_cam - \
                smpl_joint_cam[self.root_joint_idx, None]  # root-relative
            smpl_joint_cam = np.dot(rot_aug_mat, smpl_joint_cam.transpose(
                1, 0)).transpose(1, 0) / 1000  # milimeter to meter

            inputs = {'img': img, 'depth': depth}
            targets = {'orig_joint_img': h36m_joint_img, 'fit_joint_img': smpl_joint_img, 'fit_mesh_img': smpl_mesh_img,
                       'orig_joint_cam': h36m_joint_cam, 'fit_joint_cam': smpl_joint_cam, 'pose_param': smpl_pose, 'shape_param': smpl_shape}
            meta_info = {'orig_joint_valid': h36m_joint_valid, 'orig_joint_trunc': h36m_joint_trunc, 'fit_joint_trunc': smpl_joint_trunc,
                         'fit_mesh_trunc': smpl_mesh_trunc, 'is_valid_fit': float(is_valid_fit), 'is_3D': float(True)}
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        sample_num = len(outs)
        eval_result = {'mpjpe_lixel': [], 'pa_mpjpe_lixel': [],
                       'mpjpe_param': [], 'pa_mpjpe_param': []}
        for n in range(sample_num):
            ...
            eval_result['mpjpe_lixel'].append(
                np.sqrt(np.sum((pose_coord_out_h36m - pose_coord_gt_h36m)**2, 1)).mean())
            eval_result['pa_mpjpe_lixel'].append(np.sqrt(
                np.sum((pose_coord_out_h36m_aligned - pose_coord_gt_h36m)**2, 1)).mean())

            if cfg.stage == 'param':
                # h36m joint from param mesh
                mesh_out_cam = out['mesh_coord_cam'] * \
                    1000  # meter to milimeter
                pose_coord_out_h36m = np.dot(
                    self.h36m_joint_regressor, mesh_out_cam)  # meter to milimeter
                pose_coord_out_h36m = pose_coord_out_h36m - \
                    pose_coord_out_h36m[self.h36m_root_joint_idx,
                                        None]  # root-relative
                pose_coord_out_h36m = pose_coord_out_h36m[self.h36m_eval_joint, :]
                pose_coord_out_h36m_aligned = rigid_align(
                    pose_coord_out_h36m, pose_coord_gt_h36m)
                eval_result['mpjpe_param'].append(
                    np.sqrt(np.sum((pose_coord_out_h36m - pose_coord_gt_h36m)**2, 1)).mean())
                eval_result['pa_mpjpe_param'].append(np.sqrt(
                    np.sum((pose_coord_out_h36m_aligned - pose_coord_gt_h36m)**2, 1)).mean())
    ...
    def print_eval_result(self, eval_result):
        print('MPJPE from lixel mesh: %.2f mm' %
              np.mean(eval_result['mpjpe_lixel']))
        print('PA MPJPE from lixel mesh: %.2f mm' %
              np.mean(eval_result['pa_mpjpe_lixel']))

        print('MPJPE from param mesh: %.2f mm' %
              np.mean(eval_result['mpjpe_param']))
        print('PA MPJPE from param mesh: %.2f mm' %
              np.mean(eval_result['pa_mpjpe_param']))
