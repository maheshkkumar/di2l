...
from src.common.utils.preprocessing import (augmentation, load_depth, load_img,
                                            process_bbox)
...
class MSCOCO(torch.utils.data.Dataset):
    def __init__(self, transform, data_split, rgbd=False):
        ...
        self.img_path = osp.join('./datasets/i2l/data/MSCOCO', 'images')
        self.annot_path = osp.join('./datasets/i2l/data/MSCOCO', 'annotations')
        self.rootnet_output_path = osp.join('./datasets/i2l/data/MSCOCO', 'rootnet_output', 'bbox_root_coco_output.json')
        ...
        self.coco_joint_regressor = np.load(osp.join('/project/aksoy-lab/Mahesh/Courses/CMPT_766/rgbd-i2l/src/data/MSCOCO', 'J_regressor_coco_hip_smpl.npy'))
    ...
    def add_pelvis(self, joint_coord):
        ...
        pelvis[2] = joint_coord[lhip_idx, 2] * \
            joint_coord[rhip_idx, 2]  # joint_valid
        pelvis = pelvis.reshape(1, 3)
        joint_coord = np.concatenate((joint_coord, pelvis))
        return joint_coord

    def load_data(self):
        db = COCO(osp.join(self.annot_path, 'person_keypoints_' +
                  self.data_split + '2017.json'))
        ...
        if self.data_split == 'train':
            for aid in db.anns.keys():
                ...
                ).reshape(-1, 1) > 0).astype(np.float32)
                joint_img[:, 2] = 0
    ...
    def get_smpl_coord(self, smpl_param, cam_param, do_flip, img_shape):
        ...
        smpl_mesh_coord, smpl_joint_coord = self.smpl.layer['neutral'](
            smpl_pose, smpl_shape, smpl_trans)
    ...
    def get_fitting_error(self, coco_joint, smpl_mesh, cam_param, img2bb_trans, coco_joint_valid):
        ...
        # z-axis component will be removed
        coco_from_smpl = self.add_pelvis(coco_from_smpl)
        coco_from_smpl = cam2pixel(
            coco_from_smpl, cam_param['focal'], cam_param['princpt'])
        coco_from_smpl_xy1 = np.concatenate(
            (coco_from_smpl[:, :2], np.ones_like(coco_from_smpl[:, 0:1])), 1)
        coco_from_smpl[:, :2] = np.dot(
            img2bb_trans, coco_from_smpl_xy1.transpose(1, 0)).transpose(1, 0)
        coco_from_smpl[:, 0] = coco_from_smpl[:, 0] / \
            cfg.input_img_shape[1] * cfg.output_hm_shape[2]
        coco_from_smpl[:, 1] = coco_from_smpl[:, 1] / \
            cfg.input_img_shape[0] * cfg.output_hm_shape[1]

        # mask joint coordinates
        coco_joint = coco_joint[:, :2][np.tile(
            coco_joint_valid, (1, 2)) == 1].reshape(-1, 2)
        coco_from_smpl = coco_from_smpl[:, :2][np.tile(
            coco_joint_valid, (1, 2)) == 1].reshape(-1, 2)
    ...
    def __getitem__(self, idx):
        ...
        depth = None
        if self.rgbd:
            depth_path = img_path.replace('/train2017', '/dpt').replace('.jpg', '.png')
        ...
        if self.data_split == 'train':
            ...
            if do_flip:
                ...
                for pair in self.coco_flip_pairs:
                    coco_joint_img[pair[0], :], coco_joint_img[pair[1], :] = coco_joint_img[pair[1], :].copy(
                    ), coco_joint_img[pair[0], :].copy()
                    coco_joint_valid[pair[0], :], coco_joint_valid[pair[1], :] = coco_joint_valid[pair[1], :].copy(
                    ), coco_joint_valid[pair[0], :].copy()

            coco_joint_img_xy1 = np.concatenate(
                (coco_joint_img[:, :2], np.ones_like(coco_joint_img[:, :1])), 1)
            coco_joint_img[:, :2] = np.dot(
                img2bb_trans, coco_joint_img_xy1.transpose(1, 0)).transpose(1, 0)
            coco_joint_img[:, 0] = coco_joint_img[:, 0] / \
                cfg.input_img_shape[1] * cfg.output_hm_shape[2]
            coco_joint_img[:, 1] = coco_joint_img[:, 1] / \
                cfg.input_img_shape[0] * cfg.output_hm_shape[1]
            ...
            if smplify_result is not None:
                ...
                smpl_coord_img = cam2pixel(
                    smpl_coord_cam, cam_param['focal'], cam_param['princpt'])

                # x,y affine transform, root-relative depth
                smpl_coord_img_xy1 = np.concatenate(
                    (smpl_coord_img[:, :2], np.ones_like(smpl_coord_img[:, 0:1])), 1)
                ...
                smpl_coord_img[:, 1] = smpl_coord_img[:, 1] / \
                    cfg.input_img_shape[0] * cfg.output_hm_shape[1]
                smpl_coord_img[:, 2] = (
                    smpl_coord_img[:, 2] / (cfg.bbox_3d_size / 2) + 1)/2. * cfg.output_hm_shape[0]
                ...
                error = self.get_fitting_error(
                    _coco_joint_img, smpl_mesh_cam, cam_param, img2bb_trans, _coco_joint_valid)
                ...
                smpl_joint_trunc = np.zeros(
                    (self.joint_num, 1), dtype=np.float32)
                smpl_mesh_trunc = np.zeros(
                    (self.vertex_num, 1), dtype=np.float32)
                is_valid_fit = False
            ...
            smpl_joint_cam = np.dot(
                rot_aug_mat, smpl_joint_cam.transpose(1, 0)).transpose(1, 0)

            inputs = {'img': img, 'depth': depth}
            targets = {'orig_joint_img': coco_joint_img, 'fit_joint_img': smpl_joint_img, 'fit_mesh_img': smpl_mesh_img,
                       'orig_joint_cam': coco_joint_cam, 'fit_joint_cam': smpl_joint_cam, 'pose_param': smpl_pose, 'shape_param': smpl_shape}
            meta_info = {'orig_joint_valid': coco_joint_valid, 'orig_joint_trunc': coco_joint_trunc, 'fit_joint_trunc': smpl_joint_trunc,
                         'fit_mesh_trunc': smpl_mesh_trunc, 'is_valid_fit': float(is_valid_fit), 'is_3D': float(False)}
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            mesh_out_img[:, :2] = np.dot(
                bb2img_trans, mesh_out_img_xy1.transpose(1, 0)).transpose(1, 0)[:, :2]

            # z: devoxelize and translate to absolute depth
            root_joint_depth = annot['root_joint_depth']
            mesh_out_img[:, 2] = (mesh_out_img[:, 2] / cfg.output_hm_shape[0] * 2. - 1) * (
                cfg.bbox_3d_size * 1000 / 2)  # change cfg.bbox_3d_size from meter to milimeter
