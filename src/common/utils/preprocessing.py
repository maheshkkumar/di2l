...
def augmentation(img, bbox, data_split, exclude_flip=False, depth=None):
    if data_split == 'train':
        scale, rot, color_scale, do_flip = get_aug_config(exclude_flip)
    else:
        scale, rot, color_scale, do_flip = 1.0, 0.0, np.array([1,1,1]), False
    
    img, trans, inv_trans, depth = generate_patch_image(img, bbox, scale, rot, do_flip, cfg.input_img_shape, depth)
    img = np.clip(img * color_scale[None,None,:], 0, 255)
    return img, trans, inv_trans, rot, do_flip, depth

def generate_patch_image(cvimg, bbox, scale, rot, do_flip, out_shape, depth=None):
    img = cvimg.copy()
    ...
    if depth is not None:
        depth = cv2.warpAffine(depth, trans, (int(out_shape[1]), int(out_shape[0])), flags=cv2.INTER_LINEAR)
        depth = depth.astype(np.float32)

    inv_trans = gen_trans_from_patch_cv(bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], scale, rot, inv=True)

    return img_patch, trans, inv_trans, depth
