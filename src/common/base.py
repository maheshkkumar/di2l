...
from src.data.dataset import MultipleDatasets
...
#    exec('from ' + 'src.data.' + cfg.trainset_3d[i] + '.' + cfg.trainset_3d[i] + ' import ' + cfg.trainset_3d[i])
#for i in range(len(cfg.trainset_2d)):
#    exec('from ' + 'src.data.' + cfg.trainset_2d[i] + '.' + cfg.trainset_2d[i] + ' import ' + cfg.trainset_2d[i])
#exec('from ' + 'src.data.' + cfg.testset + ' import ' + cfg.testset)
from src.data.Human36M.Human36M import Human36M
from src.data.MSCOCO.MSCOCO import MSCOCO
from src.data.PW3D.PW3D import PW3D
...
class Trainer(Base):
    ...
    def _make_batch_generator(self, rgbd=False):
        # data load and construct batch generator
        self.logger.info("Creating dataset...")
        trainset3d_loader = []
        for i in range(len(cfg.trainset_3d)):
            trainset3d_loader.append(eval(cfg.trainset_3d[i])(transforms.ToTensor(), "train", rgbd=rgbd))
        trainset2d_loader = []
        for i in range(len(cfg.trainset_2d)):
            trainset2d_loader.append(eval(cfg.trainset_2d[i])(transforms.ToTensor(), "train", rgbd=rgbd))
        ...
        if len(trainset3d_loader) > 0 and len(trainset2d_loader) > 0:
            ...
            trainset3d_loader = MultipleDatasets(
                trainset3d_loader, make_same_len=False)
            trainset2d_loader = MultipleDatasets(
                trainset2d_loader, make_same_len=False)
        ...
        self.batch_generator = DataLoader(dataset=trainset_loader, batch_size=cfg.num_gpus *
                                          cfg.train_batch_size, shuffle=True, num_workers=cfg.num_thread, pin_memory=True)

    def _make_model(self, rgbd=False):
        # prepare network
        self.logger.info("Creating graph and optimizer...")
        model = get_model(self.vertex_num, self.joint_num, 'train', rgbd=rgbd)
...
class Tester(Base):
    ...
    def _make_batch_generator(self, rgbd=False):
        # data load and construct batch generator
        self.logger.info("Creating dataset...")
        testset_loader = eval(cfg.testset)(transforms.ToTensor(), "test", rgbd=rgbd)
    ...
    def _make_model(self, rgbd=False):
        ...
        model = get_model(self.vertex_num, self.joint_num, 'test', rgbd=rgbd)
        model = DataParallel(model).cuda()
